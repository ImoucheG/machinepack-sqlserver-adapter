      ...
      if (queryInfo.values && queryInfo.values.length > 0) {
        ...
        if (inputs.statement && inputs.statement.columns && inputs.statement.tableName) {
          ...
          for (let column of inputs.statement.columns) {
            ...
            if (!typesMap.find(el => el.column === column)) {
              if (queryInfo.values.length >= i && queryInfo.sql.includes('@p' + i.toString())) {
                ...
                if ((!queryInfo.sql.includes(`[${column}] is not null`) && !queryInfo.sql.includes(`[${column}] is null`)) || queryInfo.sql.includes(`[${column}] =`)
                  || queryInfo.sql.includes(`[${column}] >`) || queryInfo.sql.includes(`[${column}] <`) || queryInfo.sql.includes(`[${column}] !=`)) {
