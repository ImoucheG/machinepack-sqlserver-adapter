module.exports = {
  ...
  exits: {
    ...
    error: {
      friendlyName: 'Error to release connection'
      // outputExample: {
      //   meta: '==='
      // }
    }
  },
  fn: async function releaseConnection(inputs, exits) {
    /* let validateConnection = require('./private/validate-connection');
    if (!inputs.manager) {
      isOrmSendNativeQuery = true;
      inputs.manager = {pool: inputs.connection.pool};
      inputs.pool = inputs.connection.pool;
    }
    // Validate provided connection.
    const isValidConnection = await validateConnection({connection: inputs.connection, pool: inputs.pool, manager: inputs.manager});
    ...
    try {
      ...
      if (!inputs.connection.loggedIn) {
        return exits.error(new Error('Connection not acquired'));
      }
      if (inputs.connection.closed) {
        return exits.error(new Error('Connection already released'));
      }
    ...
    /**
     * @deprecated
     * The prepared statement release automaticaly the connection
     */
    //inputs.connection.release(inputs.connection.pool.used[0]);
    return exits.success();
  }
};
