  ...
  fn: async function sendNativeQuery(inputs, exits) {
    ...
    if (queryInfo.values && queryInfo.values.length > 0) {
      ...
      if (inputs.statement && inputs.statement.columns && inputs.statement.tableName) {
        for (let column of inputs.statement.columns) {
          ...
          if (queryInfo.sql.includes('top (@p0)') && i === 0) {
          ...
          if (queryInfo.values.length >= i && queryInfo.sql.includes('@p' + i.toString())) {
            ...
            const preparedStatement = await psGetType.prepare(`SELECT DATA_TYPE,
                                                               ...
                                                               WHERE TABLE_NAME = '` + inputs.statement.tableName + `' AND
                   COLUMN_NAME = '` + column + `'`);
      ...
      if (inputs.statement.columns.length === 0 && (queryInfo.sql.includes('top (@p0)') || queryInfo.sql.includes('offset @p0 rows'))) {
        ps.input('p0', mssql.BigInt());
        params['p0'] = queryInfo.values[0];
      }
      if (inputs.statement.columns.length === 0 && (queryInfo.sql.includes('top (@p1)') || queryInfo.sql.includes('next @p1 rows only'))) {
        ps.input('p1', mssql.BigInt());
        params['p1'] = queryInfo.values[1];
      }
    }
    ...
    if (!isOrmSendNativeQuery && result && result.recordset) {
      ...
      if (result && result.recordset) {
        normalizedNativeResult = result.recordset;
      }
    }
