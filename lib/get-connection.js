module.exports = {
  ...
  fn: async function getConnection(inputs, exits) {
    try {
      let currentPool = inputs.manager.pools.find(el => el.connected && el.pool.used.length === 0);
      if (!currentPool) {
        currentPool = inputs.manager.pools.find(el => el.connected && el.pool.used.length < 5);
      }
      if (!currentPool) {
        currentPool = inputs.manager.pools.find(el => el.connected);
      }
      if (!currentPool) {
        currentPool = inputs.manager.pools.find(el => !el.connected);
        if (currentPool) {
      ...
      if (currentPool) {
        ...
        return exits.failed({message: 'Cannot acquire a pool'});
      }
    } catch (err) {
