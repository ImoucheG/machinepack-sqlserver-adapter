  ...
  fn: async function createManager(inputs, exits) {
    ...
    try {
      ...
      if (inputs.connectionConfig) {
        if (inputs.connectionConfig.pool && inputs.connectionConfig.pool.max &&
          parseInt(inputs.connectionConfig.pool.max, 10) > 0) {
          maxPool = parseInt(inputs.connectionConfig.pool.max, 10);
        }
      }
      let pools = [];
      for (let index = 0; index < maxPool; index++) {
        createPool({connectionConfig: inputs.connectionConfig}).then(pool => { // jshint ignore:line
          pools.push(pool);
