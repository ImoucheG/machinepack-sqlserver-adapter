module.exports = require('machine').build({
  ...
  fn: async function createPool({connectionConfig}, {success}) {
    ...
    try {
      ...
      if (connectionConfig.stream) {
        sqlserverClientConfig.stream = connectionConfig.stream;
      }
      if (connectionConfig.pool && connectionConfig.pool.idleTimeoutMillis) {
        if (!sqlserverClientConfig.pool) {
          sqlserverClientConfig.pool = {};
        }
        sqlserverClientConfig.pool.idleTimeoutMillis = parseInt(connectionConfig.pool.idleTimeoutMillis, 10);
      }
      if (connectionConfig.pool && connectionConfig.pool.acquireTimeoutMillis) {
        if (!sqlserverClientConfig.pool) {
          sqlserverClientConfig.pool = {};
        }
        sqlserverClientConfig.pool.acquireTimeoutMillis = parseInt(connectionConfig.pool.acquireTimeoutMillis, 10);
      }
