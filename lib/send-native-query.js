          ...
          for (let column of inputs.statement.columns) {
            ...
            if (!typesMap.find(el => el.column === column) && sql.includes(column)) {
              if (queryInfo.values.length >= i && queryInfo.sql.includes('@p' + i.toString())) {
                ...
                if (!columnProperty) {
                  return exits.queryFailed({error: {message: `${column} not exist in table ${tableName} check your model`}});
                }
                if (columnProperty.CHARACTER_MAXIMUM_LENGTH && queryInfo.values[i] && columnProperty.DATA_TYPE !== 'xml') {
                  if (columnProperty.CHARACTER_MAXIMUM_LENGTH !== -1 && queryInfo.values[i].length > (columnProperty.CHARACTER_MAXIMUM_LENGTH + numberCharactersSpecials)) {
