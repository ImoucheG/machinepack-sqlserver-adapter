...
module.exports = {
  ...
  fn: async function sendNativeQuery(inputs, exits) {
    try {
      ...
      let ps = new mssql.PreparedStatement(inputs.connection);
      const resultParams = await getParams({
        preparedStatement: ps, statement: inputs.statement, values: inputs.valuesToEscape,
        connection: inputs.connection, sql: queryInfo.sql
      });
