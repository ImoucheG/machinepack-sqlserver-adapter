...
module.exports = {
  ...
  fn: function sendNativeQuery(inputs, exits) {
    ...
    if (queryInfo.values && queryInfo.values.length > 0) {
      let i = 0;
      for (const value of queryInfo.values) {
        let type = mssql.VarChar(mssql.Max);
        if (typeof value === 'number') {
          type = mssql.BigInt;
        }
        ps.input('p' + i.toString(), type);
        params['p' + i.toString()] = value;
        i++;
      }
    }
    ps.prepare(queryInfo.sql && !isOrmSendNativeQuery ? queryInfo.sql : queryInfo, err => {
      if (err) {
      ...
      ps.execute(params, (err, result) => {
        ...
        // inputs.manager.pool.request().query(queryInfo, function query() {
