  ...
  fn: async function sendNativeQuery(inputs, exits) {
    try {
      ...
      if (!inputs.manager) {
        ...
        inputs.manager = {connection: inputs.connection};
      }
      // Validate provided connection.
      const isValidConnection = await validateConnection({connection: inputs.connection, manager: inputs.manager})
        .catch(err => {
      ...
      const ps = new mssql.PreparedStatement(inputs.connection);
      // Add input
          ...
          for (let column of inputs.statement.columns) {
            ...
            if (!typesMap.find(el => el.column === column) && sql.includes(column)) {
              if (queryInfo.values.length >= i && queryInfo.sql.includes('@p' + i.toString())) {
                ...
                const endIndex = i;
                const parameterPart = sql.substr(sql.indexOf('@p' + startIndex), ((sql.indexOf('@p' + endIndex) + 4) - sql.indexOf('@p' + startIndex)));
                if ((!parameterPart.includes(`[${column}] is not null`) && !parameterPart.includes(`[${column}] is null`)) || parameterPart.includes(`[${column}] =`)
                  || parameterPart.includes(`[${column}] >`) || parameterPart.includes(`[${column}] <`) || parameterPart.includes(`[${column}] !=`)) {
              ...
              if (!queryInfo.sql.includes(`[${column}] is not null`) && !queryInfo.sql.includes(`[${column}] is null`)) {
                const colProperty = typesMap.find(el => el.column === column);
                if (colProperty) {
                  ps.input('p' + i.toString(), colProperty.type);
                  params['p' + i.toString()] = queryInfo.values[i];
                  i++;
                }
              }
      ...
      const result = await preparedStatement.execute(params).catch(async err => {
        await preparedStatement.unprepare().catch(err => {
          return exits.queryFailed(err);
        });
        return exits.queryFailed({
          error: err,
