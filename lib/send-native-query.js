      ...
      if (inputs.statement && inputs.statement.columns && inputs.statement.tableName) {
        for (let column of inputs.statement.columns) {
          ...
          if (queryInfo.values.length >= i && queryInfo.sql.includes('@p' + i.toString())) {
            ...
            if (columnProperty.CHARACTER_MAXIMUM_LENGTH && queryInfo.values[i]) {
                ...
                return exits.queryFailed({
                  error: {
                    message: 'Column ' + column
                      + ' is too many large with this value ' + queryInfo.values[i] + ', the length authorized is '
                      + (columnProperty.CHARACTER_MAXIMUM_LENGTH + numberCharactersSpecials)
                  }
                });
              }
            }
