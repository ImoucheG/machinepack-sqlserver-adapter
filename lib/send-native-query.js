...
module.exports = {
  ...
  fn: async function sendNativeQuery({manager, connection, meta, nativeQuery, valuesToEscape, statement}, exits) {
    try {
      ...
      const isValidConnection = await validateConnection({connection: connection, manager: manager})
        .catch(err => {
      ...
      const resultParams = await getParams({
        preparedStatement: ps, statement: statement, values: valuesToEscape, sql: queryInfo.sql, tablesProperties: manager.tablesProperties
      });
