module.exports = {
  ...
  fn: async function parseNativeQueryError({nativeQueryError, meta}, {success}) {
    ...
    if (!nativeQueryError || !nativeQueryError.code) {
      if (nativeQueryError && nativeQueryError.raw && nativeQueryError.raw.error) {
        footprint.keys = [nativeQueryError.raw.error];
      } else {
        footprint.keys = nativeQueryError.cause;
      }
      return success({
        footprint: footprint,
        meta: meta
    ...
    footprint.keys = nativeQueryError.raw && nativeQueryError.raw.precedingErrors ? nativeQueryError.raw.precedingErrors : nativeQueryError.raw ? nativeQueryError.raw.cause : nativeQueryError;
    const messagePreceding = nativeQueryError.raw && nativeQueryError.raw.precedingErrors ? nativeQueryError.raw.precedingErrors.map(el => el.message + '\n') : '';
    return success({
      footprint: footprint,
      date: new Date().toISOString(),
      message: (nativeQueryError.raw && nativeQueryError.raw.message ? nativeQueryError.raw.message : nativeQueryError.message) + '\n' + messagePreceding,
      meta: meta
    });
  }
};
