...
module.exports = require('machine').build({
  ...
  fn: async function getParams({statement, sql, preparedStatement, values, tablesProperties}, {success}) {
        ...
        for (let column of statement.columns) {
          ...
          if (!typesMap.find(el => el.column === column) && sql.includes(column)) {
            if (values.length >= i && sql.includes('@p' + i.toString())) {
              ...
              if (!columnProperty) {
                throw new Error(`${column} not exist in table ${statement.tableName} check your model`);
              }
              if (columnProperty.CHARACTER_MAXIMUM_LENGTH && currentValue && columnProperty.DATA_TYPE !== 'xml') {
                if (columnProperty.CHARACTER_MAXIMUM_LENGTH !== -1 &&
                  ...
                  throw new Error('Column ' + column + ' is too many large with this value ' + currentValue +
              ...
              if (columnProperty.CHARACTER_MAXIMUM_LENGTH < 0 || !columnProperty.CHARACTER_MAXIMUM_LENGTH) {
                columnProperty.CHARACTER_MAXIMUM_LENGTH = 2147483647;
              }
    ...
    return success({preparedStatement: preparedStatement, params});
  }
});
