...
module.exports = {
  ...
  fn: function compileStatement(inputs, exits) {
    ...
    if (compiledNativeQuery.sql.includes('top (@p0)') &&
      typeof compiledNativeQuery.bindings[compiledNativeQuery.bindings.length - 1] === 'number') {
      const top = compiledNativeQuery.bindings[compiledNativeQuery.bindings.length - 1];
      compiledNativeQuery.bindings.unshift(top);
      compiledNativeQuery.bindings.pop();
    }
